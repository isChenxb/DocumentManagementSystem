...
<body>
       <div style="margin:20px 0;"></div>
	        <div class="easyui-panel" title="公文审批" style="width:600px;padding:30px 60px;">
	        <div style="margin-bottom:20px">
	            <span>基本信息</span>
	            <div class="datagrid-toolbar" style="margin-top:5px"></div>
			</div>
       <form id="ff" method="post" action="innerApprove" enctype="multipart/form-data">
	        <div style="margin-bottom:20px">
			<span>
			...
			<div style="margin-bottom:20px">
                <input class="easyui-combobox" name="approve_result" 
                       data-options="label:'审批结果：',
                                     url: 'approve_result.json', 
                                     valueField:'approve_value',
                                     textField:'approve_text',
                                     required:true">
			</div>
			
			<div style="margin-bottom:20px">
			<span>
				<input class="easyui-textbox" name="approve_message" style="width:450px;height:100px" data-options="label:'审批意见：',multiline:true,required:true">
			</span>
			</div>
			</form>
			...
			<a href="javascript:void(0)" class="easyui-linkbutton" onclick="submitForm()" style="width:80px">提交审批</a>
		    </div>
	</div>
	<script>
	    ...
	    function submitForm(){
			$('#ff').form('submit', {
				success: function(result){
					$.messager.alert('提交结果',result,'info',dohref);
				}
			});
		}
		
		function dohref() {
			window.open('myInnerApprove','_self');
		}
	</script>
</body>
</html>